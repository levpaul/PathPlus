...
public class AStarAlgorithm<T extends State<T>> implements Algorithm<T> {
	...
	private PriorityQueue<T> openList = new PriorityQueue<T>();

	private Hashtable<Integer, T> openListHashTable = new Hashtable<Integer, T>();
	private Hashtable<Integer, T> closedList = new Hashtable<Integer, T>();
	...
	public PathResult<T> search(T start, T goal) {
		...
		openList.add(start);
		openListHashTable.put(start.getKey(), start);
	...
	private boolean forwardSearch(T goalState) {
		...
		while (openList.size() > 0) {
			...
			openListHashTable.remove(current.getKey());
			closedList.put(current.getKey(), current);
			...
			for (int i = 0; i < neighbours.length; i++) {
				...
				if (closedList.containsKey(neighbours[i].getKey())) {// check
					isInClosed = true;
				}
				...
				if (isInClosed == false) {
					...
					if (openListHashTable.containsKey(neighbours[i].getKey()))
						isInClosed = true;
				}// end if
				...
				if (isInClosed == false) {
					openListHashTable.put(neighbours[i].getKey(), neighbours[i]);
	...
	public Hashtable<Integer, T> getOpenList(){
		return openListHashTable;
	}
	
	public Hashtable<Integer, T> getClosedList(){
		return closedList;
	}
